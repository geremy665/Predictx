...
:root{
  ...
  --red4:rgba(229,9,20,.06);
  --gold:#f5a623;
  --gold2:rgba(245,166,35,.1);
  --green:#46d369;
  --green2:rgba(70,211,105,.1);
  --blue:#4da3ff;
  --blue2:rgba(77,163,255,.1);
  --t1:#ffffff;
  --t2:#e5e5e5;
  --t3:#a3a3a3;
  --t4:#525252;
  --ln:rgba(255,255,255,.08);
  --ln2:rgba(255,255,255,.04);
...
@keyframes glow{0%,100%{box-shadow:0 0 20px rgba(229,9,20,.3)}50%{box-shadow:0 0 40px rgba(229,9,20,.6)}}
.fu{animation:fu .3s ease forwards}
.si{animation:si .2s ease forwards}

/* HEADER */
header{position:sticky;top:0;z-index:100;background:rgba(10,10,10,.97);backdrop-filter:blur(20px);border-bottom:1px solid var(--ln)}
.hi{max-width:960px;margin:0 auto;display:flex;align-items:center;padding:0 16px;height:54px;gap:10px}
.logo{font-family:'Bebas Neue',sans-serif;font-size:22px;letter-spacing:2px;color:var(--t1)}
.logo span{color:var(--red)}
.hbadge{padding:3px 9px;border-radius:4px;font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:500;letter-spacing:.08em;text-transform:uppercase;background:var(--c2);color:var(--t4);border:1px solid var(--ln);white-space:nowrap}
.hbadge.ok{background:var(--green2);color:var(--green);border-color:rgba(70,211,105,.2)}
.bkbtn{margin-left:auto;display:flex;flex-direction:column;align-items:flex-end;padding:5px 12px;background:var(--c1);border:1px solid var(--ln);border-radius:8px;cursor:pointer;transition:all .15s}
.bkbtn:hover{border-color:var(--red);background:var(--red3)}
.bkl{font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--t4);text-transform:uppercase;letter-spacing:.1em}
.bkv{font-family:'Inter',sans-serif;font-size:14px;font-weight:700;color:var(--t1)}

/* NAV */
nav{background:rgba(10,10,10,.97);backdrop-filter:blur(20px);border-bottom:1px solid var(--ln);overflow-x:auto;position:sticky;top:54px;z-index:90}
.ni{max-width:960px;margin:0 auto;display:flex;padding:0 12px}
.tb{display:flex;align-items:center;gap:5px;padding:11px 12px;border:none;background:transparent;font-size:12px;font-weight:500;color:var(--t4);cursor:pointer;white-space:nowrap;border-bottom:2px solid transparent;margin-bottom:-1px;transition:all .15s;letter-spacing:.3px;text-transform:uppercase}
.tb:hover{color:var(--t2)}
.tb.on{color:var(--t1);border-bottom-color:var(--red);font-weight:600}
.tb .n{font-family:'JetBrains Mono',monospace;font-size:9px;padding:1px 5px;border-radius:3px;background:var(--c2);color:var(--t4)}
.tb.on .n{background:var(--red3);color:var(--red)}

/* PAGE */
.pg{max-width:960px;margin:0 auto;padding:20px 16px 60px}

/* CARDS */
.card{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r2);padding:18px;margin-bottom:10px}
.cardg{background:var(--c1);border:1px solid rgba(229,9,20,.3);border-radius:var(--r2);padding:18px;margin-bottom:10px}
.cardr{background:rgba(229,9,20,.06);border:1px solid rgba(229,9,20,.25);border-radius:var(--r2);padding:18px;margin-bottom:10px}
.cardem{background:var(--green2);border:1px solid rgba(70,211,105,.25);border-radius:var(--r2);padding:18px;margin-bottom:10px}
.clbl{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);text-transform:uppercase;letter-spacing:.12em;margin-bottom:12px}

/* TAGS */
.tag{display:inline-flex;align-items:center;gap:3px;padding:2px 8px;border-radius:3px;font-size:10px;font-weight:700;letter-spacing:.06em;text-transform:uppercase}
.tg{background:var(--gold2);color:var(--gold)}
.te{background:var(--green2);color:var(--green)}
.tr{background:var(--red3);color:var(--red)}
.tb2{background:var(--blue2);color:var(--blue)}

/* MATCH ROW */
.league{margin-bottom:20px}
.lg-hd{display:flex;align-items:center;gap:8px;margin-bottom:8px;cursor:pointer;padding:4px 0}
.lg-n{font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--t4);text-transform:uppercase;letter-spacing:.1em;font-weight:500}
.lg-c{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);padding:1px 6px;background:var(--c2);border-radius:3px;border:1px solid var(--ln)}
.lg-l{flex:1;height:1px;background:var(--ln)}
.lg-ar{font-size:10px;color:var(--t4);transition:transform .2s}
.lg-ar.op{transform:rotate(180deg)}

.mwrap{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r2);overflow:hidden;margin-bottom:5px}
.mwrap.hot{border-color:rgba(229,9,20,.3)}
.mtop{height:2px;display:none}
.mwrap.hot .mtop{display:block;background:var(--red)}
.mwrap.arb .mtop{display:block;background:var(--green)}
.mrow{display:grid;grid-template-columns:54px 1fr 164px 1fr 68px;align-items:center;padding:12px 14px;cursor:pointer;transition:background .1s;border-bottom:1px solid var(--ln2);position:relative}
.mrow:last-child{border-bottom:none}
.mrow:hover{background:var(--c2)}

.mtime{text-align:center}
.mt{font-family:'JetBrains Mono',monospace;font-size:12px;color:var(--t4);display:block}
.mt.hot{color:var(--red);font-weight:600}
...
.mtag.choc{color:var(--red)}.mtag.arb{color:var(--green)}

.mteam{font-size:13px;font-weight:600;color:var(--t1);letter-spacing:-.1px}
.mxg{font-family:'JetBrains Mono',monospace;font-size:9px;margin-top:2px}

.odds{display:flex;gap:3px;justify-content:center}
.odd{display:flex;flex-direction:column;align-items:center;min-width:47px;padding:6px 4px;border-radius:6px;background:var(--c2);border:1px solid var(--ln);cursor:pointer;transition:all .12s}
.odd:hover{background:var(--red3);border-color:rgba(229,9,20,.4);transform:translateY(-1px)}
.odd.val{background:var(--gold2);border-color:rgba(245,166,35,.4)}
.odd.best{background:var(--green2);border-color:rgba(70,211,105,.4)}
.odd-l{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);letter-spacing:.04em}
.odd-v{font-family:'Inter',sans-serif;font-size:14px;font-weight:700;color:var(--t1);margin-top:1px}

.mcta{display:flex;justify-content:flex-end}
.abtn{padding:5px 10px;border-radius:6px;font-size:11px;font-weight:600;border:1px solid var(--ln);color:var(--t4);background:transparent;cursor:pointer;transition:all .12s;text-transform:uppercase;letter-spacing:.3px}
.abtn:hover{border-color:var(--red);color:var(--red);background:var(--red3)}

.vbadge{position:absolute;right:72px;top:50%;transform:translateY(-50%);font-family:'JetBrains Mono',monospace;font-size:8px;font-weight:700;color:var(--green);background:var(--green2);border:1px solid rgba(70,211,105,.3);border-radius:3px;padding:2px 6px;text-transform:uppercase;letter-spacing:.06em}

/* SCANNER */
.scan-hero{text-align:center;padding:40px 16px 28px;position:relative}
.scan-hero::before{content:'';position:absolute;top:0;left:50%;transform:translateX(-50%);width:400px;height:200px;background:radial-gradient(ellipse at center,rgba(229,9,20,.08) 0%,transparent 70%);pointer-events:none}
.scan-t{font-family:'Bebas Neue',sans-serif;font-size:clamp(42px,10vw,72px);letter-spacing:3px;margin-bottom:8px;color:var(--t1);line-height:1}
.scan-t strong{color:var(--red)}
.scan-s{font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--t4);letter-spacing:.12em;text-transform:uppercase;margin-bottom:28px}
.scan-btn{height:52px;padding:0 36px;background:var(--red);border:none;border-radius:6px;font-family:'Inter',sans-serif;font-size:14px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:#fff;cursor:pointer;transition:all .18s;animation:glow 3s ease-in-out infinite}
.scan-btn:hover{background:var(--red2);transform:translateY(-2px)}
.scan-btn:disabled{background:var(--c3);color:var(--t4);box-shadow:none;transform:none;animation:none}

.sgrid{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:20px}
.sbox{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r);padding:14px;text-align:center}
.sv{font-family:'Bebas Neue',sans-serif;font-size:28px;letter-spacing:1px;color:var(--t1);line-height:1;margin-bottom:3px}
.sl{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);text-transform:uppercase;letter-spacing:.1em}

.sig{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r2);overflow:hidden;margin-bottom:8px;cursor:pointer;transition:all .15s}
.sig:hover{border-color:rgba(229,9,20,.3);transform:translateY(-2px)}
.sig.top{border-color:rgba(229,9,20,.4)}
.sig-str{height:2px;background:var(--red)}
.sig-bd{padding:16px}
.sig-mt{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);margin-bottom:8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap;text-transform:uppercase;letter-spacing:.08em}
.sig-tm{font-size:12px;color:var(--t3);margin-bottom:3px}
.sig-bt{font-family:'Inter',sans-serif;font-size:20px;font-weight:800;letter-spacing:-.5px;margin-bottom:12px;line-height:1.1;color:var(--t1)}
.sig-mg{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px}
.sig-m{background:var(--c2);border-radius:6px;padding:9px 6px;text-align:center;border:1px solid var(--ln2)}
.sig-ml{font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--t4);text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px}
.sig-mv{font-family:'Inter',sans-serif;font-size:17px;font-weight:700;letter-spacing:-.3px}
.cbar{height:2px;background:var(--c3);border-radius:1px;overflow:hidden;margin-bottom:8px}
.cbf{height:100%;border-radius:1px;background:var(--red);transition:width 1s ease}

/* COMPARATEUR */
.ctable{width:100%;border-collapse:collapse;font-size:13px}
.ctable th{padding:8px 10px;text-align:left;font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);letter-spacing:.1em;text-transform:uppercase;border-bottom:1px solid var(--ln);background:var(--c2)}
.ctable td{padding:10px 10px;border-bottom:1px solid var(--ln2)}
.ctable tr:last-child td{border-bottom:none}
.ctable tr:hover td{background:var(--c2)}
.bkn{font-family:'Inter',sans-serif;font-size:13px;font-weight:600;color:var(--t1)}
.oc{font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:500;color:var(--t3)}
.oc.best{color:var(--green);font-weight:700}
.oc.val{color:var(--gold);font-weight:700}
.pin-b{display:inline-flex;align-items:center;font-family:'JetBrains Mono',monospace;font-size:8px;font-weight:700;color:var(--blue);background:var(--blue2);border-radius:3px;padding:1px 5px;margin-left:4px;text-transform:uppercase;letter-spacing:.04em}
.avg-r td{background:var(--red3);font-weight:700}
.avg-r .oc{color:var(--red)}

/* VERDICT */
.vrd{background:var(--c1);border:1px solid rgba(229,9,20,.3);border-radius:var(--r2);padding:22px;margin-bottom:10px;position:relative;overflow:hidden}
.vrd::after{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:var(--red)}
.vey{font-family:'JetBrains Mono',monospace;font-size:9px;letter-spacing:.15em;text-transform:uppercase;color:var(--t4);margin-bottom:6px}
.vbet{font-family:'Inter',sans-serif;font-size:clamp(20px,4vw,30px);font-weight:800;letter-spacing:-1px;line-height:1.1;margin-bottom:4px;color:var(--t1)}
.vmeta{font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--t4);margin-bottom:18px}
.crow{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px}
.cl{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);text-transform:uppercase;letter-spacing:.1em}
.cv{font-family:'Inter',sans-serif;font-size:14px;font-weight:700;color:var(--t1)}
.ctr{height:3px;background:var(--c3);border-radius:2px;overflow:hidden;margin-bottom:18px}
.cf{height:100%;border-radius:2px;background:var(--red);transition:width 1.2s ease}
.prow{display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:12px}
.pb{background:var(--c2);border:1px solid var(--ln);border-radius:var(--r);padding:12px 7px;text-align:center}
.pb.win{background:var(--red3);border-color:rgba(229,9,20,.3)}
.pp{font-family:'Inter',sans-serif;font-size:24px;font-weight:800;letter-spacing:-.5px;line-height:1;margin-bottom:3px;color:var(--t1)}
.pb.win .pp{color:var(--red)}
.pn{font-size:11px;color:var(--t4)}
.pb.win .pn{color:var(--red)}
.pi{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);margin-top:2px}
.b3{height:3px;background:var(--c3);border-radius:2px;display:flex;overflow:hidden;margin-top:8px}
.b3h{height:100%;background:var(--red);transition:width 1.2s ease}
.b3n{height:100%;background:var(--c3)}
.b3a{height:100%;background:var(--gold);transition:width 1.2s ease}

/* EDGE BLOCK */
.edgb{display:flex;justify-content:space-between;align-items:center;padding:14px 16px;border-radius:var(--r);margin-bottom:10px}
.edgb.pos{background:var(--green2);border:1px solid rgba(70,211,105,.25)}
.edgb.neg{background:rgba(229,9,20,.06);border:1px solid rgba(229,9,20,.2)}
.edgv{font-family:'Inter',sans-serif;font-size:26px;font-weight:800;letter-spacing:-1px}
...
.sc2.top{background:var(--red3);border-color:rgba(229,9,20,.3)}
.scv{font-family:'Inter',sans-serif;font-size:17px;font-weight:700;letter-spacing:-.3px}
.sc2.top .scv{color:var(--red)}
.scp{font-family:'JetBrains Mono',monospace;font-size:10px;color:var(--t4);margin-top:2px}

/* MARCHÉS */
.mg4{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-bottom:10px}
.mk{background:var(--c2);border:1px solid var(--ln);border-radius:10px;padding:11px 7px;text-align:center}
.mk.val{background:var(--gold2);border-color:rgba(245,166,35,.3)}
.mk.ok{background:var(--green2);border-color:rgba(70,211,105,.3)}
.mkp{font-family:'Inter',sans-serif;font-size:18px;font-weight:700;color:var(--t1);letter-spacing:-.3px}
.mk.val .mkp{color:var(--gold)}.mk.ok .mkp{color:var(--green)}
.mkl{font-size:10px;color:var(--t4);margin-top:3px}
.mke{font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;margin-top:2px}
.mk.val .mke{color:var(--gold)}.mk.ok .mke{color:var(--green)}

/* BARRES */
.br{margin-bottom:8px}
.brt{display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px}
.brb{height:3px;background:var(--c3);border-radius:2px;overflow:hidden}
.brf{height:100%;border-radius:2px;transition:width 1.2s ease}

/* KELLY */
.kg{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-bottom:10px}
.kb{background:var(--c2);border:1px solid var(--ln);border-radius:10px;padding:12px;text-align:center}
.kbv{font-family:'Inter',sans-serif;font-size:20px;font-weight:700;color:var(--red);letter-spacing:-.3px}
.kbl{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);margin-top:3px;text-transform:uppercase;letter-spacing:.1em}
.vp{padding:9px 12px;border-radius:8px;font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:.3px}
.vp.y{background:var(--green2);border:1px solid rgba(70,211,105,.25);color:var(--green)}
.vp.n{background:rgba(229,9,20,.06);border:1px solid rgba(229,9,20,.2);color:var(--red)}

/* FORMS */
.fw{display:flex;flex-direction:column;gap:5px;margin-bottom:8px}
.fl{font-family:'JetBrains Mono',monospace;font-size:9px;color:var(--t4);text-transform:uppercase;letter-spacing:.1em}
.fi{width:100%;padding:10px 12px;background:var(--c2);border:1px solid var(--ln);border-radius:8px;color:var(--t1);font-size:14px;outline:none;transition:border-color .15s}
.fi:focus{border-color:var(--red);box-shadow:0 0 0 2px var(--red3)}
.fi.big{padding:12px 13px;font-size:15px;font-weight:500}
.fsel{width:100%;padding:10px 12px;background:var(--c2);border:1px solid var(--ln);border-radius:8px;color:var(--t1);font-size:14px;outline:none;appearance:none}
.rg{display:flex;gap:3px}
.rb{flex:1;padding:7px 4px;font-size:11px;font-weight:500;border-radius:6px;border:1px solid var(--ln);background:var(--c2);color:var(--t4);cursor:pointer;transition:all .12s}
.rb:hover{border-color:var(--red);color:var(--red)}
.rb.on{background:var(--red3);border-color:rgba(229,9,20,.4);color:var(--red);font-weight:700}
...
.fsec{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r2);padding:16px;margin-bottom:8px}
.fsh{display:flex;align-items:center;gap:8px;margin-bottom:12px;padding-bottom:10px;border-bottom:1px solid var(--ln2)}
.fsn{font-family:'JetBrains Mono',monospace;font-size:9px;font-weight:700;color:var(--red);background:var(--red3);border-radius:4px;padding:2px 7px;text-transform:uppercase;letter-spacing:.08em}
.fst{font-size:13px;font-weight:600;color:var(--t1)}

/* AI BAR */
.aib{background:var(--c1);border:1px solid rgba(229,9,20,.2);border-radius:var(--r2);padding:16px;margin-bottom:10px;position:relative;overflow:hidden}
.aib::before{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,var(--red),transparent)}
.aih{font-size:13px;font-weight:700;color:var(--t1);margin-bottom:4px;display:flex;align-items:center;gap:6px;letter-spacing:-.1px;text-transform:uppercase}
.aid{width:6px;height:6px;border-radius:50%;background:var(--red);box-shadow:0 0 8px var(--red)}
.ais{font-size:12px;color:var(--t4);margin-bottom:12px;line-height:1.6}
.cbtn{width:100%;height:48px;background:var(--red);border:none;border-radius:8px;font-family:'Inter',sans-serif;font-size:13px;font-weight:700;letter-spacing:.5px;text-transform:uppercase;color:#fff;cursor:pointer;transition:all .18s;margin-bottom:8px}
.cbtn:hover{background:var(--red2);transform:translateY(-1px);box-shadow:0 8px 24px rgba(229,9,20,.3)}
.cbtn:disabled{background:var(--c3);color:var(--t4);box-shadow:none;transform:none}
.stb{padding:8px 11px;background:var(--red3);border:1px solid rgba(229,9,20,.2);border-radius:6px;font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--red);margin-bottom:8px;display:flex;align-items:center;gap:7px}
.aimsg{padding:8px 11px;border-radius:6px;font-family:'JetBrains Mono',monospace;font-size:11px;margin-top:6px}
.aiok{background:var(--green2);border:1px solid rgba(70,211,105,.2);color:var(--green)}
.aier{background:rgba(229,9,20,.06);border:1px solid rgba(229,9,20,.2);color:var(--red)}
.msel{background:var(--red3);border:1px solid rgba(229,9,20,.2);border-radius:10px;padding:10px 13px;margin-bottom:10px;display:flex;align-items:center;gap:10px}

/* MODAL */
.modal{position:fixed;inset:0;z-index:200;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;padding:16px;backdrop-filter:blur(8px)}
.mbox{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r3);padding:24px;width:100%;max-width:400px;position:relative;overflow:hidden}
.mbox::before{content:'';position:absolute;top:0;left:0;right:0;height:2px;background:var(--red)}
.mtitle{font-family:'Inter',sans-serif;font-size:17px;font-weight:700;margin-bottom:16px;letter-spacing:-.3px;color:var(--t1)}
.mrow2{display:flex;gap:8px;margin-top:14px}

/* BANKROLL */
.sg2{display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-bottom:16px}
.hr{display:flex;justify-content:space-between;align-items:center;padding:10px 0;border-bottom:1px solid var(--ln2)}
.hr:last-child{border-bottom:none}

/* TIPS */
.tip{background:var(--c1);border:1px solid var(--ln);border-radius:var(--r2);overflow:hidden;margin-bottom:8px;cursor:pointer;transition:all .15s}
.tip:hover{border-color:rgba(229,9,20,.3);transform:translateY(-1px)}
.tip.top{border-color:rgba(229,9,20,.35)}
.tph{padding:8px 15px;background:var(--c2);display:flex;justify-content:space-between;align-items:center;border-bottom:1px solid var(--ln2)}
.tpb{padding:14px 15px}
.tpbt{font-family:'Inter',sans-serif;font-size:19px;font-weight:800;margin-bottom:9px;letter-spacing:-.4px;color:var(--t1)}
.tpmg{display:grid;grid-template-columns:repeat(4,1fr);gap:5px;margin-bottom:8px}
.tpm{background:var(--c2);border-radius:6px;padding:8px 5px;text-align:center;border:1px solid var(--ln2)}
.tpml{font-family:'JetBrains Mono',monospace;font-size:8px;color:var(--t4);text-transform:uppercase;letter-spacing:.08em;margin-bottom:3px}
.tpmv{font-family:'Inter',sans-serif;font-size:15px;font-weight:700;letter-spacing:-.3px}

/* EMPTY */
.empty{text-align:center;padding:52px 20px}
.ei{font-size:36px;opacity:.1;margin-bottom:12px}
.et{font-family:'Inter',sans-serif;font-size:18px;font-weight:700;color:var(--t3);margin-bottom:6px;letter-spacing:-.3px}
.es{font-size:13px;color:var(--t4);line-height:1.7}

.ldr{display:flex;gap:4px;justify-content:center;align-items:center}
.ldr span{width:5px;height:5px;border-radius:50%;background:var(--red);opacity:.6}
.spin{width:16px;height:16px;border:2px solid rgba(229,9,20,.15);border-top-color:var(--red);border-radius:50%;animation:pu 0.7s linear infinite}
.disc{padding:10px 13px;background:rgba(229,9,20,.06);border:1px solid rgba(229,9,20,.15);border-radius:8px;font-size:11px;color:var(--t4);line-height:1.8;margin-top:4px}

.fils{display:flex;gap:5px;overflow-x:auto;padding-bottom:2px;margin-bottom:14px}
.fib{padding:5px 12px;border-radius:4px;font-size:11px;font-weight:600;border:1px solid var(--ln);background:transparent;color:var(--t4);cursor:pointer;transition:all .12s;white-space:nowrap;text-transform:uppercase;letter-spacing:.3px}
.fib:hover{border-color:rgba(229,9,20,.4);color:var(--red)}
.fib.on{background:var(--red);border-color:var(--red);color:#fff}

@media(max-width:520px){
  .mrow{grid-template-columns:46px 1fr 144px 1fr 56px;padding:10px 11px}
  .odd{min-width:41px}.odd-v{font-size:13px}
...
[{"c":"La Liga","h":"Real Madrid CF","a":"Atlético de Madrid","t":"21:00","bet":"Victoire Real Madrid","type":"1x2","odd":2.02,"prob":0.58,"edge":0.13,"conf":"high","val":true,"top":true,"kelly":0.04,"pinOdd":1.95,"reason":"xG dom 2.38, 6V/7 à domicile. Edge vs Pinnacle +5%.","risk":"Derby"}]`,2000);
  ...
  function Resultat(){
    ...
    const cc=conf>=75?"var(--red)":conf>=60?"var(--t1)":"var(--t3)";
